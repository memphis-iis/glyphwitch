...
import { cosignSimilarity } from 'mathjs';
...
Meteor.methods({
    ...
    //set user role
    setUserRole: async function(username, role) {
    ...
    changeEmailPassword: async function(oldEmail, newPassword, newEmail=False) {
    ...
    addDocument: async function(file, title, author) {
    ...
    updateDocument: async function(document, file, title, author) {
    ...
    addPageToDocument: async function(document, page) {
    ...
    removePageFromDocument: async function(document, page) {
    ...
    modifyPageInDocument: async function(document, page) {
    ...
    addLineToPage: async function(document, page_number, x1, y1, width, height) {
    ...
    removeLineFromPage: async function(document, page, line) {
    ...
    addWordToLine: async function(document, page, line, x, width, wordOrder=false, word=false) {
    ...
    removeWordFromLine: async function(document, page, line, wordOrder) {
    ...
    modifyWordInLine: async function(document, page, line, wordOrder, word) {
    ...
    addReference: async function(document, page, line, wordOrder, word, notes) {
    ...
    removeReference: async function(reference) {
    ...
    updateReference: async function(reference, document, page, line, wordOrder, word, notes) {
    ...
    addPhonemeToReference: async function(reference, phoneme) {
    ...
    removePhonemeFromReference: async function(reference, phoneme) {
    ...
    addPhoneme: async function(phoneme, ipa) {
    ...
    removePhoneme: async function(phoneme) {
    ...
    updatePhoneme: async function(phoneme, phoneme, ipa) {
    ...
    addGlyphToPhoneme: async function(phoneme, glyph) {
    ...
    removeGlyphFromPhoneme: async function(phoneme, glyph) {
    ...
    addGlyph: async function(glyph, font=False, unicode=False, image=False) {
    ...
    addGlyphFromDataURL: async function(dataURL, font="", unicode=0) {
    ...
    addGlyphFromBuffer: async function(buffer, font, unicode) {
    ...
    removeGlyph: async function(glyph) {
    ...
    updateGlyph: async function(glyph, glyph, font, unicode, image) {
    ...
    //add a set of glyphs from a image zip file
    addGlyphsFromZip: async function(file) {
        console.log("Adding glyphs from zip (file: " + file + ")");
        //get the file from the image collection
        file = Files.findOne({ _id: file });
        //unzip the file
        const zip = new require('node-zip')();
        const zipData = fs.readFileSync(file.path);
        const unzipped = zip.unzip(zipData);
        //iterate over all the files in the zip
        for (file in unzipped) {
            console.log("Processing file: " + file);
            //if the file is an image, add it as a glyph
            if (file.includes(".png")) {
                console.log("Adding glyph from file: " + file);
                const glyph = Files.write(unzipped[file], { fileName: file, type: 'image/png' });
                Meteor.callAsync('addGlyphFromBuffer', glyph, "", 0);
            }
        }
    },
    //remove an entry from the fonts collection. Must include the font id.
    removeFont: async function(font) {
    ...
    findReferences: async function(document, page=False, line=False, wordOrder=False) {
    ...
    addDiscussion: async function(reference, comment) {
    ...
    getDiscussion: async function(reference) {
...
function blurImage(imageData, kernelSize) {
  ...
  function createGlyphImage(fontPath, charCode) {
    ...
    Meteor.callAsync('addGlyphFromDataURL', newBuffer, fontFileName, charCode);
  }